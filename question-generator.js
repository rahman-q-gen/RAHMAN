(() => {
  ...
  function detectBestMapping(rows, relationships) {
    ...
    for (const map of candidates) {
      ...
      for (const row of rows) {
        ...
        if (normalizeAnswer(cellText(cells[map.ans]))) score += 5;
  ...
  function recordFromSourceRow(row, relationships, map, ordinal) {
    ...
    const answer = normalizeAnswer(cellText(answerCell));
  ...
  function cellHasMeaningfulContent(cell, relationships) {
    ...
    if (cell.getElementsByTagNameNS(M_NS, "oMath").length) return true;
    if (cell.getElementsByTagNameNS(W_NS, "drawing").length || cell.getElementsByTagNameNS(W_NS, "pict").length) return true;
  ...
  async function generateSets(mode) {
    ...
    try {
      ...
      for (let index = 0; index < setCount; index += 1) {
        ...
        let shuffled = secureShuffle(sourceRecords.slice()).slice(0, questionsPerSet);
