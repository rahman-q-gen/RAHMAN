(() => {
  ...
  function detectBestMapping(rows, relationships) {
    ...
    for (const map of candidates) {
      ...
      for (const row of rows) {
        ...
        const serialText = map.serial === null ? "" : cellText(cells[map.serial]);
        const mappedCells = [cells[map.q], cells[map.a], cells[map.b], cells[map.c], cells[map.d], cells[map.ans], cells[map.exp]];
        const hasAnything = mappedCells.some((cell) => cellHasMeaningfulContent(cell, relationships)) || Boolean(serialText);
        ...
        if (map.serial !== null && /^[0-9০-৯]+[.)।-]*$/.test(serialText)) score += 2;
      }
      score += considered;
  ...
  function recordFromSourceRow(row, relationships, map, ordinal) {
    ...
    const rowHasContent = mappedCells.some((cell) => cellHasMeaningfulContent(cell, relationships)) || Boolean(serialText);
  ...
  function cellHasMeaningfulContent(cell, relationships) {
    ...
    // Cell-based validation: visible Word content means the field exists.
    // This includes normal text, Symbol-font characters such as Ω (w:sym),
    // equations, images, objects, tabs/line breaks and other visible markers.
    const textNodes = [
  ...
  function validateSourceRecords(records) {
    ...
    for (const record of records) {
      ...
      if (record.answerMissing) {
        blockingErrors.push(`${number} নম্বর প্রশ্নে সঠিক Answer (A/B/C/D) নেই। Answer যুক্ত করে Generate করুন।`);
      }
  ...
  async function generateSets(mode) {
    ...
    try {
      ...
      resultsNote.textContent = `${sourceRecords.length}টি source প্রশ্ন থেকে ${setCount}টি Set তৈরি হয়েছে · প্রতি Set-এ ${questionsPerSet}টি প্রশ্ন · Mode: ${modeLabel} · Answer ও Explanation সঠিক প্রশ্নের সঙ্গে রাখা হয়েছে।`;
  ...
  function createDistinctQuestionOrder(records, takeCount, usedOrders) {
    ...
    // Deterministic fallback for small banks: try non-zero rotations.
    for (let shift = 1; shift < source.length; shift += 1) {
      const candidate = source.slice(shift).concat(source.slice(0, shift)).slice(0, count);
      if (questionOrderHasSerialCollision(candidate)) continue;
    ...
    throw new Error(
      "Admin serial-এর সঙ্গে generated serial আলাদা রাখা এবং প্রতিটি Set-এর question order আলাদা রাখার শর্তে আর নতুন Set তৈরি করা সম্ভব হয়নি। Set সংখ্যা কমান অথবা Question Bank-এ আরও প্রশ্ন যোগ করুন।"
    );
  }
  ...
  function questionOrderHasSerialCollision(records) {
    return records.some((record, index) => {
      ...
      return sourceNumber !== null && sourceNumber === index + 1;
    });
  }
  ...
  function sourceSerialNumber(record) {
    ...
    const parsed = parseQuestionSerial(record.sourceNumber);
  ...
  function shuffleRecordOptions(record) {
    ...
    // If the Answer cell contains content but it cannot be interpreted as A/B/C/D,
    // keep this question's option order unchanged rather than corrupting its answer.
