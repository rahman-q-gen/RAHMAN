(() => {
  ...
  async function prepareSourceAndAutoGenerate() {
    ...
    try {
      ...
      detectedInfoEl.textContent = `✓ ${count}টি প্রশ্ন পাওয়া গেছে${mathCount ? ` · ${mathCount}টি প্রশ্নে Word equation আছে` : ""}${imageCount ? ` · ${imageCount}টি প্রশ্নে embedded image আছে` : ""}`;
  ...
  async function parseQuestionBank(arrayBuffer) {
    ...
    for (const table of tables) {
      ...
      for (const row of rows) {
        const record = recordFromSourceRow(row, relationships);
  ...
  function recordFromSourceRow(row, relationships) {
    ...
    for (const map of mappings) {
      ...
      if ((!question && !imageRelIds.length) || options.filter(Boolean).length < 2 || !answer) continue;
