    ...
    try {
      ...
      for (let index = 0; index < setCount; index += 1) {
        ...
        for (const [kind, suffix] of [["question", "Questions"], ["answer", "Answers"], ["merged", "Combined"]]) {
          outputs[kind] = { name: `${base} - ${suffix}.docx`, blob: await buildSetDocx(setLabel, shuffled, kind) };
          generatedFiles.push(outputs[kind]);
        }
        const set = { setLabel, count: shuffled.length, outputs, preview: shuffled.map((record) => ({
      ...
      resultsNote.textContent = `${sourceRecords.length}টি বৈধ প্রশ্ন থেকে ${setCount}টি shuffled Set তৈরি হয়েছে · প্রতি Set-এ ${questionsPerSet}টি প্রশ্ন${sourceIssues.length ? ` · ${sourceIssues.length}টি অবৈধ উত্তরযুক্ত প্রশ্ন বাদ দেওয়া হয়েছে` : ""}।`;
